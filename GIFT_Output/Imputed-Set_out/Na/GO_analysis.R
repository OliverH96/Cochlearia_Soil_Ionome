...
setwd("C://Users/Oliver/Desktop/UoN/Individual_project/GWAS_out/impute_out/Na/")
# Open my files
...
top_perc <- read.csv("C://Users/Oliver/Desktop/UoN/Individual_project/GWAS_out/impute_out/Na/genesofinterest.txt", 
                     header = FALSE)
...
# First convert top_perc from a dataframe to a vector
top_perc_vector <- as.vector(top_perc[,'V1'])

# Then convert genesInScan to a vector
genesInScan_vector <- as.vector(genesInScan[,'V1'])

# Then make the factor
allGenes_factor <- factor(as.integer(genesInScan_vector %in% top_perc_vector))
