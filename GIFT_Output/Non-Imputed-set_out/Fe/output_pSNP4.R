...
library("htmlwidgets")
#library("manhattanly")
GWAS_result1 <- read.csv("C://Users/Oliver/Desktop/UoN/Individual_project/GWAS_out/no_impute_out/Fe/output.csv")
GWAS_result1[GWAS_result1==Inf] <- NA
GWAS_result1[GWAS_result1==-Inf] <- NA
GWAS_result<-GWAS_result1[complete.cases(GWAS_result1),]
# Calculate the BHY threshold
m <- nrow(GWAS_result)
GWAS_result <- GWAS_result[order(GWAS_result$pSNP4),]
s <- 1.0
i <- 0
for (p in GWAS_result$pSNP4) {
  p
  i <- i+1
  if (i > 1) {
    s <- s + 1.0/(i-1)
...
ylim <- abs(floor(log10(min(GWAS_result$pSNP4)))) +1
png("C://Users/Oliver/Desktop/UoN/Individual_project/GWAS_out/no_impute_out/Fe/output_pSNP4.png", bg = "white", width = 9.75, height = 3.25, units = "in", res = 1200, pointsize = 4)
manhplot <- ggplot(GWAS_result, aes(x = bp_cum, y = (-log10(pSNP4)), # size = 1, 
...
dev.off()
#attach(GWAS_result)
#GWAS_result <- GWAS_result[order(-pSNP4),]
#detach(GWAS_result)
...
#GWAS_result$P <- (0.0000001/GWAS_result$pSNP4)
#names(GWAS_result)[names(GWAS_result) == "POS"] <- "BP"
#html_file <- manhattanly(GWAS_result, annotation1 = "CHR", annotation2 = "BP")
#saveWidget(html_file, "C://Users/Oliver/Desktop/UoN/Individual_project/GWAS_out/no_impute_out/Fe/output_pSNP4.html", selfcontained = T)
